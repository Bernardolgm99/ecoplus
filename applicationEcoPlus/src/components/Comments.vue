        ...
        <div class="d-flex mx-auto w-75" v-for="(comment, index) in comments">
            <div class="d-flex flex-column align-center">
                <v-btn variant="text" icon="mdi-arrow-up-bold" size="x-large"
                    :color="comment.userRated.rated && comment.userRated.value ? 'green' : 'grey'" @click="like(comment)">
                </v-btn>
                <span> {{ comment.rate.like - comment.rate.dislike }} </span>
                <v-btn variant="text" icon="mdi-arrow-down-bold" size="x-large"
                    :color="comment.userRated.rated && !comment.userRated.value ? 'red' : 'grey'" @click="dislike(comment)">
                </v-btn>
            </div>
            <div class="d-flex flex-column justify-center w-100">
                <div class="d-flex justify-space-between align-baseline">
                    ...
                    <div v-if="comment.creator">
                        ...
                        <div v-else>
                            <v-btn color="red" variant="text" class="ms-3 mb-6" icon="mdi-close" flat="true" size="small"
                                @click="toggleEditComment(index)"></v-btn>
                            <v-btn color="green" variant="text" class="ms-3 mb-6" icon="mdi-check" flat="true" size="small"
                                @click="editComment(comment)"></v-btn>
                        </div>
                    </div>
                </div>
